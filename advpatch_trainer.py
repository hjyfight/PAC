...
def build_training_data(samples, patch_size, image_size=(640, 640), resize_mode='squash'):
    """
...
def main():
    ...
    ap.add_argument('--resize_mode', choices=['squash', 'letterbox'], default='squash',
                    help="Training resize mode: 'squash' uses direct 640x640 resize; "
                         "'letterbox' uses YOLOv5 letterbox before placing patches.")
    ...
    training_data = build_training_data(samples, args.patch_size, resize_mode=args.resize_mode)
    ...
    for epoch in tqdm(range(args.num_iterations), desc="AdvPatch"):
        ...
        for b in range(0, n, args.batch_size):
            ...
            for i in idxs:
                ...
                img_t = image_to_training_tensor(img, resize_mode=args.resize_mode).to(device)
                patch = patch_module()
                img_with = img_t
